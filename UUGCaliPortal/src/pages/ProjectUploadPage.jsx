...
export default function ProjectUploadPage() {
  ...
  const handleEditProject = (item) => {
    ...
    setFormData({
      id: item.id || '',
      title: item.title || '',
      author: item.author || 'CaliUUG Team',
      category: item.category || 'General',
      description: item.description || '',
      status: item.status || 'Prototipo',
  ...
  const handleSaveProject = (e) => {
    ...
    const projectSlug = formData.title.toLowerCase().replace(/[^a-z0-9]/g, '-');
    const newId = editingId || (formData.id.trim() ? formData.id.trim() : `proj-${projectSlug || Date.now()}`);

    const projectPayload = {
      id: newId,
      title: formData.title,
      author: formData.author || 'CaliUUG Team',
      category: formData.category || 'General',
      description: formData.description || '',
      status: formData.status || 'Prototipo',
    ...
    let updatedList;
    if (editingId) {
      updatedList = fullJsonData.map((item) => (item.id === editingId ? projectPayload : item));
      setStatusMsg({ type: 'info', text: 'Proyecto actualizado localmente. Recuerda enviar el commit.' });
    } else {
      updatedList = [projectPayload, ...fullJsonData];
      setStatusMsg({ type: 'info', text: 'Proyecto agregado a la lista local. Haz clic en "Guardar Cambios" para subir a GitHub.' });
    }
  ...
  const handleRawJsonChange = (e) => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        setJsonError('El JSON debe ser un Arreglo [] de proyectos.');
      }
    } catch (err) {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <section className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="font-['Inter'] text-lg text-[#45464d] max-w-2xl mt-2">
            Administra, crea y edita los proyectos destacados contenidos en <code className="font-['JetBrains_Mono'] text-black font-semibold">projects.json</code>.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="bg-white/40 backdrop-blur-md border border-black/10 rounded-xl p-12 text-center my-8">
          ...
          <p className="font-['JetBrains_Mono'] text-sm text-[#45464d]">Cargando proyectos...</p>
        </div>
      ) : (
            ...
            <form onSubmit={handleSaveProject} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  name="description"
                  rows={3}
                  placeholder="Resumen del proyecto, alcance y metas..."
              ...
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-12 sm:col-span-6">
                  ...
                  <TagInput
                    ...
                    placeholder="Sci-Fi, Unity, Low-Poly..."
                  />
                </div>
                <div className="col-span-12 sm:col-span-6">
                  <label className="block font-['JetBrains_Mono'] text-xs uppercase tracking-wider text-black font-bold mb-1">
                  ...
                  <TagInput
                    ...
                    placeholder="C#, HLSL, ShaderGraph..."
                  />
                </div>
              </div>
          ...
          <div className="col-span-12 lg:col-span-6 flex flex-col gap-6">
            <div className="flex items-center justify-between bg-white/40 backdrop-blur-md border border-black/10 rounded-xl p-3">
              <span className="font-['JetBrains_Mono'] text-xs font-bold text-black uppercase tracking-wider pl-2">
                Proyectos en Lista ({fullJsonData.length})
              </span>
              <button
                type="button"
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="font-['JetBrains_Mono'] text-[11px] text-[#45464d]">
                                {item.category || 'General'} • {item.status || 'Prototipo'} ({item.version})
                              </p>
                            </div>
                          </div>
                        ...
                        {isOpen && (
                          <div className="px-4 pb-4 pt-2 border-t border-black/5 font-['Inter'] text-xs text-[#45464d] space-y-2 bg-white/30">
                            ...
                            <p><strong>Autor:</strong> {item.author} | <strong>Licencia:</strong> {item.license}</p>
                            {item.githubUrl && (
                              <p><strong>Repo:</strong> <code className="font-['JetBrains_Mono'] text-black">{item.githubUrl}</code></p>
                            )}
                            ...
                            {item.technologies && item.technologies.length > 0 && (
                              <div className="flex flex-wrap gap-1 pt-1">
                                {item.technologies.map((tech, idx) => (
                                  <span key={idx} className="bg-emerald-50 text-emerald-900 font-['JetBrains_Mono'] text-[10px] px-2 py-0.5 rounded border border-emerald-200">
                                    {tech}
                                  </span>
                                ))}
                              </div>
